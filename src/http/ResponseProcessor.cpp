...
ResponseProcessor::ResponseProcessor(const HttpRequest& request, std::vector<ServerConfig> serverVector)
{
	_httpResponse = buildResponse(request, serverVector);
}

ResponseProcessor::ResponseProcessor(const ResponseProcessor & other) :
...
ResponseProcessor & ResponseProcessor::operator=(const ResponseProcessor & other)
{
	if (&other != this) {
...
const HttpResponse & ResponseProcessor::buildResponse(const HttpRequest& request, const std::vector<ServerConfig> serverVector)
{
...
void ResponseProcessor::buildRedirect(int statusCode, const ServerConfig & server, const Location * location)
{
...
void ResponseProcessor::buildError(int statusCode, const ServerConfig & server, const Location * location)
{
	std::string filePath = selectErrorPage(statusCode, server, location);
	std::string body;
	std::string mimeType;
...
const HttpResponse & ResponseProcessor::getHttpResponse() const
{
	return (_httpResponse);
}
...
void ResponseProcessor::addMandatoryHeaders(const ServerConfig & server, size_t bodySize)
{
